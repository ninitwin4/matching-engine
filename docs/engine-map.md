...
LLM call costs.)
ANSWER:  Not all results need AI since it has the base deterministic score that way LLM cost save for unnecessary results and plus AI bonus is capped in code so it won't reach within that capped.
...
rather than showing the average one. Just because A has 90 doesn't mean, it would be an average match to B who score has 20, it would clear clash once they start living under the same roof. 
