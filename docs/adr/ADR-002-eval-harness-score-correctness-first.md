...
**Harness design (Steps 3–4, decided same session):**

- **Two-speed runner.** Fast lane: deterministic cases as parametrized pytest
  (free, run on every change). Slow lane: custom runner for the AI suite
  (N repeated LLM calls per case; run on demand). Rationale: an eval suite
  expensive enough to avoid running is worthless.
- **Per-case checks (AI suite):** bounds (±10 after parsing), direction,
  groundedness (rationale cites required bio facts), consistency
  (max − min adjustment ≤ max_spread). Raw outputs, latencies, and token
  costs are recorded per run — making the harness double as the measurement
  instrument for the later optimization phase.
- **Reports:** timestamped markdown files committed to `evals/reports/`;
  failures include raw runs and rationales (the input to failure analysis);
  headline metrics (pass rates, mean spread, cost) are diffable run-over-run.
- **Pass bar: 100% deterministic / 100% AI suite — any red blocks changes.**
  N = 3 repeat runs per AI case.
- **Red-run triage protocol (required by the strict bar):** red = investigate,
  never auto-dismiss, never delete a case to make it pass. Re-run once: pass →
  log as a flake (recurring flakes on one case indicate a consistency
  problem); fail again → real; decide with evidence whether the fix is the
  prompt, the weights, or the case.
